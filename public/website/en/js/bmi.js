function calc()
{
	...
	{
		document.BMI.desc.value=d + "Obese Class 1";
	}
    else if(si>35 && si<=40)
	{
		document.BMI.desc.value=d + "Obese Class 2";
	}
     else if(si>40 && si<=50)
	{
		document.BMI.desc.value=d + "More To Obese";
	}
      else if(si>50)
	{
		document.BMI.desc.value=d + "Super More To Obese";
	}
}



function calcIdealWeight(h) {
    return 25 * (h * h) / 10000;
}
var bmiIdeal = document.querySelector('.bmi-ideal');

document.querySelector('.bmi-form').addEventListener('submit', submitBMI);
function submitBMI(e) {
    bmiIdeal.textContent = 'Your Ideal Weight is :  ' + (calcIdealWeight(document.querySelector('#your-height').value));
    e.preventDefault();
}
